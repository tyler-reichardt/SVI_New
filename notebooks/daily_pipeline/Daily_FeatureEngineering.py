...
table_path = get_table_path("single_vehicle_incident_checks", "daily_claims_pol_svi", "mlstore")
try:
...
if preprocessed_df:
    ...
    feature_engineer.spark.sql(f"CREATE SCHEMA IF NOT EXISTS {env_config['mlstore_catalog']}.single_vehicle_incident_checks")

    # Write with partitioning
    df.write.mode("overwrite").option("overwriteSchema", "true").partitionBy("dataset").saveAsTable(f"{env_config['mlstore_catalog']}.single_vehicle_incident_checks.daily_svi_features")
