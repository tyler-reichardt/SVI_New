...
# Save as a global temp view
check_df.createOrReplaceGlobalTempView("check_df_global")
...
# Save as a global temp view
policy_svi.createOrReplaceGlobalTempView("policy_svi_global")
...
other_cols = ['claim_number', 'policy_number', 'policy_transaction_id',  'start_date', 'policy_start_date']
...
display(spark.createDataFrame([(check_df.count(), len(check_df.columns))], ["num_rows", "num_columns"]))

# COMMAND ----------

display(spark.createDataFrame([(col,) for col in sorted(check_df.columns)], ["column_name"]))

# COMMAND ----------

display(check_df.filter(col("claim_number") == "FC/901099994").select(*sorted(check_df.columns)))
